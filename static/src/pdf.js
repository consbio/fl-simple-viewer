    ...
    function processMaps(callback) {
        try {
            leafletImage(map, function(err, canvas) {
                if (err) {
                    console.error("Caught error in leafletImage");
                    console.error(err);
                }
            ...
            if (err) {
                console.error("Caught error executing leafletImage");
                console.error(err);
            }
        }
    }
            ...
            if (textContent) {
                ...
                texts.push({
                    ...
                    style: {
                        ...
                        fontStyle:
                            style.fontWeight >= 700 || style.fontWeight.toLowerCase() === "bold" ? "bold" : "normal",
                        color: color
                    }
                });
                ...
                pdf.addHTML(
                    ...
                    function() {
                        texts.forEach(function(t) {
                            ...
                            if (t.url) {
                                pdf.textWithLink(t.text, t.pos.x - 3, t.pos.y + 17 * (1 - t.pos.y / 792), {
                                    url: t.url
                                });
                            } else {
